...
def _resolve_life_moment(
    ...
    *,
    ...
    safe_candidates = [moment for moment in moments if moment.get("category") in safe_categories]
    if safe_candidates:
        return sorted(safe_candidates, key=lambda moment: _slugify(moment.get("moment", "")))[0], "safe_category_fallback"
    return None, "unresolved_no_safe_fallback"
...
def validate_resolved_content_brief(brief: ResolvedContentBrief) -> List[Dict[str, str]]:
    ...
    if brief.life_moment_id and brief.life_moment_text:
        results.append(_validation("pass", "life_moment", "Canonical pain point resolved to a life moment."))
    else:
        results.append(_validation("critical failure", "life_moment", "No category-aligned life moment was resolved."))
...
def validate_asset_metadata(brief: ResolvedContentBrief, asset_metadata: Dict[str, Any]) -> List[Dict[str, str]]:
    ...
    incompatible = (brief.slot == "morning" and asset_time in {"night", "evening"}) or (
        brief.slot == "evening" and asset_time == "morning"
    )
    if incompatible:
...
def log_resolved_content_brief(brief: ResolvedContentBrief) -> None:
...
def resolve_content_brief(
    *,
    ...
    if presentation["engagement_prompt_enabled"]:
        selected_prompt = content_engine.get_random_engagement_prompt()
